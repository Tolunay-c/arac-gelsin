...
<!-- DESIGN 2: klasik iletişim topbar'ı kaldırıldı — yerine yüzen, hap
     şeklinde tek bir gezinme çubuğu var (bkz. alt-theme.css .site-header
     override'ı). Tema anahtarı işlevini korumak için buraya taşındı. -->
  ...
  <div class="container">
    ...
    <nav class="main-nav" id="mainNav" aria-label="Ana menü">
      <ul>
        ...
        <?php endforeach; ?>
        <!-- CTA, ul içinde: dar ekranda hamburger menüsüyle birlikte
             katlanır — sabit üst çubukta yer kaplamaz (bkz. taşan mobil
             düzeltmesi). Geniş ekranda .main-nav a.nav-cta ile hap
             butona dönüşür. -->
    ...
    <div class="alt-header-actions">
      <button type="button" class="theme-toggle" id="themeToggle" aria-label="Açık/koyu temayı değiştir">
        <?= icon('sun', 'icon-sun') ?>
        <?= icon('moon', 'icon-moon') ?>
      </button>
