...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        if ($data['title'] === '') {
            ...
            UseCase::create($data);
            flash_set('success', 'Senaryo eklendi.');
        } else {
            UseCase::update($id, $data);
            flash_set('success', 'Senaryo güncellendi.');
        }
    } elseif ($action === 'delete') {
        UseCase::delete($id);
        flash_set('success', 'Senaryo silindi.');
    } elseif ($action === 'toggle') {
        UseCase::toggleActive($id);
    }

    redirect('use-cases.php');
}

$editing = isset($_GET['edit']) ? UseCase::find((int) $_GET['edit']) : null;
$items = UseCase::all();
...
<div class="admin-panel">
  <div class="admin-panel__header"><h2><?= $editing ? 'Senaryoyu Düzenle' : 'Yeni Senaryo Ekle' ?></h2></div>
  ...
  <form method="post" class="admin-form-grid">
    ...
    <label class="admin-field">İkon
      <?= icon_select('icon', $editing['icon'] ?? 'car') ?>
    </label>
    ...
    <div class="admin-form-actions">
      ...
      <?php if ($editing): ?><a href="use-cases.php" class="btn-admin btn-admin--ghost">Vazgeç</a><?php endif; ?>
    </div>
  </form>
</div>

<div class="admin-panel">
  <div class="admin-panel__header"><h2>Tüm Senaryolar</h2></div>
  <?php if ($items === []): ?>
    <p class="admin-empty">Henüz senaryo eklenmedi.</p>
  <?php else: ?>
